...
secp256k1_context* DlcUtil::GetSecpContext() {
...
bool DlcUtil::SchnorrVerify(const Pubkey& pubkey, const Pubkey& nonce,
  ...
  return secp256k1_schnorrsig_verify(GetSecpContext(), &sig,
...
Pubkey DlcUtil::GetCommittedKey(const Pubkey& oracle_pub_key,
  ...
  int ret = secp256k1_ec_pubkey_combine(
      GetSecpContext(), &combined_key, pointer_list.data(), pubkey_list.size());
...
secp256k1_pubkey DlcUtil::GetSecpCommittedKey(const Pubkey& oracle_pub_key,
  ...
  int ret = secp256k1_schnorrsig_sig_pubkey(
      GetSecpContext(), &result, &rp, message_hash.GetBytes().data(), &pk);

  if (ret != 1) {
    throw CfdException(CfdError::kCfdInternalError,
                       "Secp256k1 sig pubkey error.");
  }
...
Privkey DlcUtil::GetTweakAddPrivateKey(const Privkey& privkey,
  ...
  int result = secp256k1_ec_privkey_tweak_add(
      GetSecpContext(), privkey_bytes.data(), tweak.GetBytes().data());
  if (!result) {
    throw CfdException(CfdError::kCfdInternalError,
                       "Error tweaking private key.");
  }
...
Pubkey DlcUtil::GetSchnorrPublicNonce(const Privkey& nonce) {
  ...
  secp256k1_schnorrsig_get_public_nonce(GetSecpContext(), &pubnonce,
                                        &schnorrnonce);
